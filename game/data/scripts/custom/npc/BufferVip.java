...
import static org.l2jdevs.gameserver.util.Util.formatAdena;
...
public class BufferVip extends Quest
{
	...
	private static final int NPC_ID = 111008;
	
	private static final String TITLE_NAME = "..:: BufferVip ::..";
	...
	private static final int BUFF_REMOVE_PRICE = 0;
	private static final int HEAL_PRICE = 0;
	private static final int BUFF_PRICE = 0;
	private static final int RESIST_PRICE = 0;
	private static final int SONG_PRICE = 0;
	private static final int DANCE_PRICE = 0;
	private static final int CHANT_PRICE = 0;
	private static final int OTHERS_PRICE = 0;
	private static final int SPECIAL_PRICE = 0;
	private static final int CUBIC_PRICE = 0;
	private static final int BUFF_SET_PRICE = 0;
	private static final int SCHEME_BUFF_PRICE = 0;
	...
	private String rebuildMainHtml(QuestState st)
	{
		...
		if (st.getInt("Pet-On-Off") == 1)
		{
			...
			MAIN_HTML_MESSAGE += "<button value=\"Pet Options\" action=\"bypass -h Quest " + QUEST_LOADING_INFO + " buffpet 0 0 0\" width=200 height=25 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\">";
		}
		else
		{
			bottonA = "Auto Buff";
			...
			MAIN_HTML_MESSAGE += "<button value=\"Player Options\" action=\"bypass -h Quest " + QUEST_LOADING_INFO + " buffpet 1 0 0\" width=200 height=25 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\">";
		}
		...
		if (MESSAGE.length() > 0)
		{
			MAIN_HTML_MESSAGE += "<table cellspacing=0 cellpadding=0>" + MESSAGE + "</table><br><br><center><img src=\"L2UI_CH3.herotower_deco\" width=256 height=32></center><br>";
			MESSAGE = "";
			td = 0;
		}
		...
		MAIN_HTML_MESSAGE += "<br></body></html>";
	...
	private String generateScheme(QuestState st)
	{
		...
		{
			PreparedStatement rss = con.prepareStatement("SELECT * FROM buffer_scheme_player WHERE player_id=?");
		...
		HTML += "<BR1><table cellspacing=0 cellpadding=0>";
	...
	{
		return "<html><head><title>" + TITLE_NAME + "</title></head><body><center><img src=\"L2UI_CH3.herotower_deco\" width=256 height=32><br>" + "<img src=\"L2UI.SquareGray\" width=250 height=1><br>" + "<table width=260 border=0 bgcolor=444444>" + "<tr><td><br></td></tr>"
			...
			+ "<tr><td></td></tr></table><br>" + "<img src=\"L2UI.SquareGray\" width=250 height=1><br><br>" + "<button value=\"Yes\" action=\"bypass -h Quest " + QUEST_LOADING_INFO + " reloadscript 1 0 0\" width=200 height=25 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\">"
			+ "<button value=\"No\" action=\"bypass -h Quest " + QUEST_LOADING_INFO + " reloadscript 0 0 0\" width=200 height=25 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\">" + "</center></body></html>";
	}
	...
	private int getBuffCount(String scheme)
	{
		...
		try (Connection con = ConnectionFactory.getInstance().getConnection())
		{
			PreparedStatement rss = con.prepareStatement("SELECT buff_class FROM buffer_scheme_list WHERE scheme_id=?");
	...
	private String getBuffType(int id)
	{
		...
		try (Connection con = ConnectionFactory.getInstance().getConnection())
		{
			PreparedStatement act = con.prepareStatement("SELECT buffType FROM buffer_vip_list WHERE buffId=? LIMIT 1");
			act.setInt(1, id);
	...
	private boolean isEnabled(int id, int level)
	{
		...
		try (Connection con = ConnectionFactory.getInstance().getConnection())
		{
			PreparedStatement act = con.prepareStatement("SELECT canUse FROM buffer_vip_list WHERE buffId=? AND buffLevel=? LIMIT 1");
			act.setInt(1, id);
			act.setInt(2, level);
			ResultSet rs = act.executeQuery();
			if (rs.next())
	...
	private boolean isUsed(String scheme, int id, int level)
	{
		...
		try (Connection con = ConnectionFactory.getInstance().getConnection())
		{
			PreparedStatement rss = con.prepareStatement("SELECT id FROM buffer_scheme_list WHERE scheme_id=? AND skill_id=? AND skill_level=? LIMIT 1");
			rss.setString(1, scheme);
			rss.setInt(2, id);
			rss.setInt(3, level);
			ResultSet action = rss.executeQuery();
	...
	private int getClassBuff(String id)
	{
		...
		try (Connection con = ConnectionFactory.getInstance().getConnection())
		{
			PreparedStatement getTipo = con.prepareStatement("SELECT buff_class FROM buffer_vip_list WHERE buffId=?");
	...
	private BufferVip()
	{
		super(-1, BufferVip.class.getSimpleName(), "custom/npc");
		addStartNpc(NPC_ID);
	...
	private String createScheme()
	{
		return "<html><head><title>" + TITLE_NAME + "</title></head><body><center><img src=\"L2UI_CH3.herotower_deco\" width=256 height=32><br><br>You MUST seprerate new words with a dot (.)<br><br>Scheme name: <edit var=\"name\" width=100><br><br>"
			+ "<button value=\"Create Scheme\" action=\"bypass -h Quest " + QUEST_LOADING_INFO + " create $name no_name x x\" width=150 height=25 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\">" + "<br><button value=\"Back\" action=\"bypass -h Quest " + QUEST_LOADING_INFO
			+ " redirect main 0 0\" width=100 height=25 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\"></center></body></html>";
	}
	...
	private String deleteScheme(L2PcInstance player)
	{
		...
		HTML += "<br><button value=\"Back\" action=\"bypass -h Quest " + QUEST_LOADING_INFO + " redirect main 0 0\" width=100 height=25 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\"></center></body></html>";
		return HTML;
	}
	...
	private String editScheme(L2PcInstance player)
	{
		...
		HTML += "<br><button value=\"Back\" action=\"bypass -h Quest " + QUEST_LOADING_INFO + " redirect main 0 0\" width=150 height=25 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\">" + "</center></body></html>";
		return HTML;
	}
	...
	private String getOptionList(String scheme)
	{
		...
		HTML += "<br><button value=\"Back\" action=\"bypass -h Quest " + QUEST_LOADING_INFO + " edit_1 0 0 0\" width=100 height=25 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\">" + "<button value=\"Home\" action=\"bypass -h Quest " + QUEST_LOADING_INFO
			+ " redirect main 0 0\" width=100 height=25 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\">" + "<br></center></body></html>";
		return HTML;
	}
	...
	private String buildHtml(String buffType)
	{
		...
		try (Connection con = ConnectionFactory.getInstance().getConnection())
		{
			PreparedStatement getList = con.prepareStatement("SELECT buffId,buffLevel FROM buffer_vip_list WHERE buffType=\"" + buffType + "\" AND canUse=1  ORDER BY Buff_Class ASC, id");
		...
		else
		{
			...
			else
			{
				...
				HTML_MESSAGE += "All special buffs cost <font color=LEVEL>" + formatAdena(price) + "</font> adenas!";
			}
		...
		HTML_MESSAGE += "<br><button value=\"Back\" action=\"bypass -h Quest " + QUEST_LOADING_INFO + " redirect main 0 0\" width=100 height=25 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\"></center></body></html>";
		return HTML_MESSAGE;
	}
	...
	private String viewAllSchemeBuffs(String scheme, String page, String action)
	{
		...
		try (Connection con = ConnectionFactory.getInstance().getConnection())
		{
			if (action.equals("add"))
			{
				...
				String QUERY = "SELECT * FROM buffer_vip_list WHERE buffType IN (" + generateQuery(BUFF_COUNT, DANCE_SONG) + ") AND canUse=1 ORDER BY Buff_Class ASC, id";
			...
			else if (action.equals("remove"))
			{
				...
				String QUERY = "SELECT * FROM buffer_scheme_list WHERE scheme_id=? ORDER BY Buff_Class ASC, id";
				PreparedStatement getBuffCount = con.prepareStatement(QUERY);
		...
		HTML_MESSAGE += "<br><br><button value=\"Back\" action=\"bypass -h Quest " + QUEST_LOADING_INFO + " manage_scheme_select " + scheme + " x x\" width=150 height=25 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\">" + "<button value=\"Home\" action=\"bypass -h Quest "
			+ QUEST_LOADING_INFO + " redirect main 0 0\" width=150 height=25 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\">" + "<br></center></body></html>";
		return HTML_MESSAGE;
	}
	...
	private String viewAllBuffTypes()
	{
		...
		HTML_MESSAGE += "<button value=\"Back\" action=\"bypass -h Quest " + QUEST_LOADING_INFO + " redirect main 0 0\" width=100 height=25 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\"></center></body></html>";
		return HTML_MESSAGE;
	}
	...
	private String viewAllBuffs(String type, String typeName, String page)
	{
		...
		try (Connection con = ConnectionFactory.getInstance().getConnection())
		{
			...
			if (type.equals("set"))
			{
				getBuffCount = con.prepareStatement("SELECT * FROM buffer_vip_list WHERE buffType IN (" + generateQuery(0, 0) + ") AND canUse=1");
			}
			else
			{
				getBuffCount = con.prepareStatement("SELECT * FROM buffer_vip_list WHERE buffType=?");
		...
		HTML_MESSAGE += "<br><br><button value=\"Back\" action=\"bypass -h Quest " + QUEST_LOADING_INFO + " redirect manage_buffs 0 0\" width=100 height=25 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\">" + "<button value=\"Home\" action=\"bypass -h Quest " + QUEST_LOADING_INFO
			+ " redirect main 0 0\" width=150 height=20 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\"></center></body></html>";
		return HTML_MESSAGE;
	}
	...
	private void manageSelectedBuff(String buffPosId, String canUseBuff)
	{
		...
		try (Connection con = ConnectionFactory.getInstance().getConnection())
		{
			PreparedStatement upd = con.prepareStatement("UPDATE buffer_vip_list SET canUse=? WHERE buffId=? AND buffLevel=? LIMIT 1");
	...
	private String manageSelectedSet(String id, String newVal, String opt3)
	{
		...
		try (Connection con = ConnectionFactory.getInstance().getConnection())
		{
			PreparedStatement upd = con.prepareStatement("UPDATE buffer_vip_list SET forClass=? WHERE buffId=? AND bufflevel=?");
	...
	public String onAdvEvent(String event, L2Npc npc, L2PcInstance player)
	{
		...
		switch (eventParam0)
		{
			...
			case "create":
			{
				...
				try (Connection con = ConnectionFactory.getInstance().getConnection())
				{
					PreparedStatement ins = con.prepareStatement("INSERT INTO buffer_scheme_player (player_id,scheme_name) VALUES (?,?)");
			...
			case "delete":
				try (Connection con = ConnectionFactory.getInstance().getConnection())
				{
					...
					rem = con.prepareStatement("DELETE FROM buffer_scheme_player WHERE id=? LIMIT 1");
					rem.setString(1, eventParam1);
					rem.executeUpdate();
					rem.close();
					rem = con.prepareStatement("DELETE FROM buffer_scheme_list WHERE scheme_id=?");
					rem.setString(1, eventParam1);
					rem.executeUpdate();
					rem.close();
				}
			...
			case "delete_c":
				return "<html><head><title>" + TITLE_NAME + "</title></head><body><center><img src=\"L2UI_CH3.herotower_deco\" width=256 height=32><br>Do you really want to delete '" + eventParam2 + "' scheme?<br><br>" + "<button value=\"Yes\" action=\"bypass -h Quest " + QUEST_LOADING_INFO
					...
					+ " delete_1 x x x\" width=50 height=25 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\"></center></body></html>";
			...
			case "remove_buff":
			{
				...
				try (Connection con = ConnectionFactory.getInstance().getConnection())
				{
					PreparedStatement rem = con.prepareStatement("DELETE FROM buffer_scheme_list WHERE scheme_id=? AND skill_id=? AND skill_level=? LIMIT 1");
			...
			case "add_buff":
			{
				...
				try (Connection con = ConnectionFactory.getInstance().getConnection())
				{
					PreparedStatement ins = con.prepareStatement("INSERT INTO buffer_scheme_list (scheme_id,skill_id,skill_level,buff_class) VALUES (?,?,?,?)");
			...
			case "cast":
				if ((int) (System.currentTimeMillis() / 1000) > st.getInt("blockUntilTime"))
				{
					...
					try (Connection con = ConnectionFactory.getInstance().getConnection())
					{
						PreparedStatement rss = con.prepareStatement("SELECT * FROM buffer_scheme_list WHERE scheme_id=? ORDER BY id");
			...
			case "castBuffSet":
				if ((int) (System.currentTimeMillis() / 1000) > st.getInt("blockUntilTime"))
				{
					...
					if (!getSummonbuff)
					{
						try (Connection con = ConnectionFactory.getInstance().getConnection())
						{
							PreparedStatement getSimilarNameCount = con.prepareStatement("SELECT buffId,buffLevel FROM buffer_vip_list WHERE forClass IN (?,?) ORDER BY id ASC");
